...
def CDIEGO_MINST(W, N, d, vti, tot_iter, x_samples, pca_vect, step_size, Tc):
    ...
    for sample in range(0, tot_iter):
        ...
        if (W_e1[:, 0] == 0).any():
            ...
            W_e1 = W_e1[:,
                   0]  # + 1e-100 # as we are taking first column only. We need to add small eps to avoid scaling by 1/0
        # Update eigenvector estimate
        for i_n in range(0, N):
...
A = (1 / 60000) * np.matrix(
    data_concatenated.T @ data_concatenated)  # Find covariance matrix Sigma using \Sigma = \tilde{A}^T \tilde{A}
...
def monte_carlo_mp_DIEGO(r):
    ...
    diego_mc_p = np.zeros(
        (total_count, tot_iter))  # Generate empty matrix to store the values of data for current thread
    for im in range(0, total_count):
        print('Current Monte Carlo index for DIEGO is: ', r[im], ' and index is: ', im, '\n')
        diego_mc_p[im, :] = DIEGO_MINST(W_f, N, d, vti, tot_iter, x_samples[r[im], :, :], pca_vect,
                                        step_size)  # Note: r[im] will be unique monte-carlo index
    return diego_mc_p
...
def monte_carlo_mp_CDIEGO_Ra(r):
    total_count = r[-1] - r[0] + 1  # Count total number entries in range
    cdiego_Ra_mc_p = np.zeros(
        (total_count, tot_iter))  # Generate empty matrix to store the values of data for current thread
    for im in range(0, total_count):
        print('Current Monte Carlo index for CDIEGO R_a is: ', r[im], ' and index is: ', im, '\n')
        cdiego_Ra_mc_p[im, :] = CDIEGO_MINST(W_nf, N, d, vti, tot_iter, x_samples[r[im], :, :], pca_vect, step_size,
                                             R_a)
...
def monte_carlo_mp_CDIEGO_Rmax(r):
    total_count = r[-1] - r[0] + 1  # Count total number entries in range
    cdiego_R_max_mc_p = np.zeros(
        (total_count, tot_iter))  # Generate empty matrix to store the values of data for current thread
    for im in range(0, total_count):
        print('Current Monte Carlo index for CDIEGO R_max is: ', r[im], ' and index is: ', im, '\n')
        cdiego_R_max_mc_p[im, :] = CDIEGO_MINST(W_nf, N, d, vti, tot_iter, x_samples[r[im], :, :], pca_vect, step_size,
                                                R_max)
...
if __name__ == '__main__':
    ...
    pool = mp.Pool(2)
    result_DIEGO = pool.imap(monte_carlo_mp_DIEGO, ranges)
    result_CDIEGO_Ra = pool.imap(monte_carlo_mp_CDIEGO_Ra, ranges)
    result_CDIEGO_Rmax = pool.imap(monte_carlo_mp_CDIEGO_Rmax, ranges)
    pool.close()
    pool.join()
    ...
    filename_data = 'sim_data/MP_DIEGO_CDIEGO_MNIST_amarel_iter_count_' + str(tot_iter) + '_dimdata_' + str(
        d) + '_nodes_' + str(
