...
diego_f_cnnc_m_ega = np.zeros((monte_carlo,tot_iter))
cdiego_f_cnnc_m_ega = np.zeros((monte_carlo,tot_iter))
diego_f_cnnc_m_egb = np.zeros((monte_carlo,tot_iter))
cdiego_f_cnnc_m_egb = np.zeros((monte_carlo,tot_iter))
[pca_vect_ega, Sigma_ega, eig_gap_a] = data_cov_mat_gen(d,eig_gap_fac_a) # Generate Cov Matrix and true eig vec
[pca_vect_egb, Sigma_egb, eig_gap_b] = data_cov_mat_gen(d,eig_gap_fac_b) # Generate Cov Matrix and true eig vec
...
for mon in range(0,monte_carlo):
    ...
    x_samples_a = np.random.multivariate_normal(np.zeros(d), Sigma_ega,N*tot_iter)
    x_samples_b = np.random.multivariate_normal(np.zeros(d), Sigma_egb, N * tot_iter)
    #%% Run DIEGO Algorithm
    diego_f_cnnc_m_ega[mon,:] = DIEGO_Hyp_Tc(W_f,N,d,vti,tot_iter,x_samples_a,pca_vect_ega,step_size)
    diego_f_cnnc_m_egb[mon, :] = DIEGO_Hyp_Tc(W_f, N, d, vti, tot_iter, x_samples_b, pca_vect_egb, step_size)
    #%% Run CDIEGO Algorithm with different communication rounds
    cdiego_f_cnnc_m_ega[mon,:] = CDIEGO_Hyp_Tc(W_nf,N,d,vti,tot_iter,x_samples_a,pca_vect_ega,step_size,R_max)
    cdiego_f_cnnc_m_egb[mon, :] = CDIEGO_Hyp_Tc(W_nf, N, d, vti, tot_iter, x_samples_b, pca_vect_egb, step_size, R_max)
#%% Save All Results
np.save('sim_data/eff_ega_iter_count_' + str(tot_iter) + '_dimdata_' + str(d) + '_nodes_' + str(
        N) + '_eg_' + str(eig_gap_fac_a) + '_ss_' + str(step_size) + '_mc_' + str(monte_carlo) + '_FC_'+'.npy', diego_f_cnnc_m_ega)
np.save('sim_data/eff_ega_iter_count_' + str(tot_iter) + '_dimdata_' + str(d) + '_nodes_' + str(
        N) + '_eg_' + str(eig_gap_fac_a) + '_ss_' + str(step_size) + '_mc_' + str(monte_carlo) + '_R_'+str(R_max)+'_NFC_'+'.npy', cdiego_f_cnnc_m_ega)
np.save('sim_data/eff_egb_iter_count_' + str(tot_iter) + '_dimdata_' + str(d) + '_nodes_' + str(
        N) + '_eg_' + str(eig_gap_fac_b) + '_ss_' + str(step_size) + '_mc_' + str(monte_carlo) + '_FC_'+'.npy', diego_f_cnnc_m_egb)
np.save('sim_data/eff_egb_iter_count_' + str(tot_iter) + '_dimdata_' + str(d) + '_nodes_' + str(
        N) + '_eg_' + str(eig_gap_fac_b) + '_ss_' + str(step_size) + '_mc_' + str(monte_carlo) + '_R_'+str(R_max)+'_NFC_'+'.npy', cdiego_f_cnnc_m_egb)
