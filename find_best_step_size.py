...
cdiego_f_cnnc_m = np.zeros((len(step_size),monte_carlo,tot_iter))
...
for st in range(0,len(step_size)):
    ...
    cdiego_f_cnnc = np.squeeze(np.array(np.mean(cdiego_f_cnnc_m[st,:,:], axis=0)))
    # Plot the curve
    plt.semilogy(diego_f_cnnc, label='DIEGO, StepSize='+str(step_size[st])+', N= '+str(N)+', gap= '+str(eig_gap),linestyle='dashed',linewidth=2)
    plt.semilogy(cdiego_f_cnnc, label='CDIEGO, StepSize='+str(step_size[st])+', N= '+str(N)+', gap= '+str(eig_gap),linestyle='dashed',linewidth=2)
