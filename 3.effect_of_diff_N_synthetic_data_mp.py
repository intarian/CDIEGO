...
# performance of C-DIEGO algorithm under FC graph using CDIEGO using Synthetic Data.
...
#%% Generate Fully Connected Graph for different Nodes
W_f_Na = np.matrix(1 / Na * (np.ones((Na, 1)) @ np.ones((Na, 1)).T))
W_f_Nb = np.matrix(1 / Nb * (np.ones((Nb, 1)) @ np.ones((Nb, 1)).T))
W_f_Nc = np.matrix(1 / Nc * (np.ones((Nc, 1)) @ np.ones((Nc, 1)).T))
