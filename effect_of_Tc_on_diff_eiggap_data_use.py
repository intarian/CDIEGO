...
plt.semilogy(cdiego_f_cnnc_rmax_eg1[start_t:end_t], label='NFC gap = '+str(eig_gap_eg1)+' Tc = '+ ' $T_{mix} (3/2) \log(Nt)$',linestyle='dashed',linewidth=1,marker='o',markersize=7, markevery=markers_cdiego.tolist())
# Plot eg 2
...
markers_cdiego = (np.ceil(np.linspace(start_t+5000,end_t-5000,5))).astype(int)
plt.semilogy(cdiego_f_cnnc_rmax_eg2[start_t:end_t], label='NFC gap = '+str(eig_gap_eg2)+' Tc = '+ ' $T_{mix} (3/2) \log(Nt)$',linestyle='dashed',linewidth=1,marker='<',markersize=7, markevery=markers_cdiego.tolist())
